...
int flowTMVA(const std::string inFileName, TString myMethodList = "" )
{
  ...
  std::string outFileName = "flowTMVA_" + std::to_string(date->GetDate()) + ".root";
  delete date;
  ...
  //  dataloader->AddVariable( "phoEt", "Photon Transverse Energy", "GeV", 'F' );
  //  dataloader->AddVariable( "phoEta", "Pseudorapidity", "", 'F' );
...
{
  if(argc != 2){
    std::cout << "Usage: ./flowTMVA.exe <inFileName>" << std::endl;
    return 1;
  }

  int retVal = 0;
  retVal += flowTMVA(argv[1]);
  return retVal;
}
